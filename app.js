...
function normalizeInternational(i={}){i=i||{};return{sourceLanguage:i.sourceLanguage||"pt-BR",targetLanguage:i.targetLanguage||"en",targetMarket:i.targetMarket||"",mode:i.mode||"singable",draft:i.draft||null,versions:Array.isArray(i.versions)?i.versions:[],selectedVersionId:i.selectedVersionId||"",reviewText:i.reviewText||"",reviewAcceptedAt:i.reviewAcceptedAt||"",plusInterest:Boolean(i.plusInterest),primeInterest:Boolean(i.primeInterest),clip:i.clip||null}}
...
async function gerarTodasImagensClipe() {
  ...
  if (btn) { btn.disabled = false; btn.textContent = '🎬 Gerar imagens de todas as cenas'; }
  renderClipScenes();
}

window.gerarCapaObra = gerarCapaObra;
window.gerarTodasImagensClipe = gerarTodasImagensClipe;
...
function generateInternationalAdaptation(){let w=getActiveWork();if(!w&&state.works.length)activeWorkId=state.works[0].id,w=getActiveWork();if(!w){if(!state.works.length)createWork();w=getActiveWork()}if(!w){const out=$("#internationalOutput");if(out)out.textContent="Crie uma obra no Escritório para gerar adaptação internacional.";if(window.cvToast)window.cvToast("Crie uma obra no Escritório primeiro.","aviso");return}updateInternationalSettings();const lines=w.lyrics.split("\n").map(l=>l.trim()).filter(Boolean);const source=lines.length?lines:firstSongLines(w);const lang=w.international.targetLanguage;const mode=w.international.mode;const market=w.international.targetMarket.trim()||marketForLanguage(lang);const btn=$("#generateInternationalButton");const out=$("#internationalOutput");if(btn){btn.disabled=true;btn.textContent="Traduzindo..."}if(out)out.textContent="Maestro está adaptando sua música para "+intlNome(lang)+"...";translateWithGroq(source.join("\n"),lang,mode,market,w.title).then(adapted=>{const score=scoreInternational(source,mode);w.international.targetMarket=market;w.international.draft={id:createId(),sourceLanguage:w.international.sourceLanguage,targetLanguage:lang,targetMarket:market,mode,source:source.join("\n"),adapted,score,createdAt:new Date().toISOString()};w.international.reviewText=adapted;touchWork(w,"Adaptacao internacional gerada para "+lang);renderInternational();renderReadiness();renderWorks()}).catch(err=>{if(out)out.textContent="Erro na tradução: "+err.message;}).finally(()=>{if(btn){btn.disabled=false;btn.textContent="Gerar adaptação"}})}
...
function intlCodigo(v){return window.Internacional?.codigoFromLegacy?.(v)||v}
function intlNome(v){return window.Internacional?.findLang?.(intlCodigo(v))?.nome||v}
function marketForLanguage(lang){const c=intlCodigo(lang);return{en:"Estados Unidos / Reino Unido",es:"Mexico / Espanha",fr:"Franca / Canada",it:"Italia",de:"Alemanha",ja:"Japao",ko:"Coreia do Sul",ar:"Oriente Medio",hi:"India","zh-CN":"China",ru:"Leste Europeu","pt-BR":"Brasil","pt-PT":"Portugal"}[c]||"Mercado internacional"}
...
/* ════════════════════════════════════════════════════════════════
   TRADUÇÃO MUSICAL REAL — Groq API via servidor
   Suporta 184 idiomas / nações
   A chave GROQ_API_KEY fica apenas no backend /api/translate
════════════════════════════════════════════════════════════════ */
const IDIOMAS_MUNDO = {
...
async function translateWithGroq(lyrics,lang,mode,market,title){
  const targetLanguage=intlNome(lang);
  const sourceLanguage=intlNome(getActiveWork()?.international?.sourceLanguage||"pt-BR");
  const style=MODOS_DESCRICAO[mode]||mode;
  try {
    const resp = await fetch('/api/translate',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({text:lyrics,targetLanguage,sourceLanguage,style})
    });
    if(resp.ok){
