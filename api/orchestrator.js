...
function buildHash(type,payload){return crypto.createHash('sha256').update(JSON.stringify({type,...payload},Object.keys({type,...payload}).sort())).digest('hex').slice(0,32);}

/* ── CAMADA 3: Pool rotativo ── */
async function getAvailableKey(provider){
  if(!SUPABASE_URL||!SUPABASE_SERVICE_KEY){if(provider==='groq'&&GROQ_API_KEY)return{id:'env-groq',key:GROQ_API_KEY};if(provider==='pollinations')return{id:'pollinations-free',key:'free'};return null;}
  try{const resp=await sbFetch('/rest/v1/api_keys?provider=eq.'+provider+'&is_active=eq.true&select=id,api_key,requests_today,daily_limit&order=last_used.asc.nullsfirst&limit=1');if(!resp.ok)return null;const rows=await resp.json();if(!rows.length)return null;const row=rows[0];if(row.daily_limit&&row.requests_today>=row.daily_limit)return null;return{id:row.id,key:row.api_key};}catch{return null;}
}
...
async function callMyMemory(payload){const{lyrics,targetLang='en'}=payload;const langMap={ingles:'en',espanhol:'es',frances:'fr',italiano:'it',alemao:'de',japones:'ja',coreano:'ko'};const to=langMap[targetLang]??targetLang.slice(0,2);const resp=await fetch('https://api.mymemory.translated.net/get?q='+encodeURIComponent(lyrics.slice(0,500))+'&langpair=pt|'+to,{signal:AbortSignal.timeout(8000)});if(!resp.ok)throw new Error('MyMemory HTTP '+resp.status);const data=await resp.json();const text=data.responseData?.translatedText;if(!text)throw new Error('MyMemory vazio');return{result:text,provider:'mymemory'};}
